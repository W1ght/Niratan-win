...
class AdapterStructureTest(unittest.TestCase):
    ...
    def test_main_worker_only_uses_registry(self) -> None:
        ...
        # 它要挡的东西，但也算行；当前主干为 721 行，因此用 722 作为独占上界，
        # 真正的 registry / TryHook 判据本身不变。
    ...
    def test_leaf_identity_does_not_latch_an_unmeasured_executable(self) -> None:
        ...
        self.assertLess(guard, latch, "测量失败必须在写入永久缓存之前就返回")
        self.assertLess(
            guard,
        ...
        self.assertIn(
            "InterlockedIncrement(&g_leaf_identity_measure_attempts)", body
        )
    ...
    def test_leaf_structure_gate_reads_the_pristine_file_not_process_memory(
        ...
        self.assertNotIn(
            "LoadLibraryEx",
            loader,
            "加载器会把本进程主映像别名回已被改写的那份",
        )
        self.assertNotIn("SEC_IMAGE", loader)
