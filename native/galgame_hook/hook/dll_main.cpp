...
// 传给注入工作线程和分层查词窗口的模块句柄。窗口线程不能依赖 DllMain 的参数，
// 因为它是在 HookWorker 完成映射后才创建的。
...
DWORD WINAPI HookWorker(LPVOID module_context) {
  ...
  g_module = reinterpret_cast<HMODULE>(module_context);
  ...
  if (g_header != nullptr) {
    ...
    if (g_header->magic == kSharedMagic &&
      ...
      if (MH_Initialize() == MH_OK) {
        ...
        // Siglus 没有可挂载查词卡片的游戏 Layer，使用同一 v15 BGRA 帧协议的
        // borderless layered presenter。KiriKiri 由其自己的 Layer presenter 负责。
        if (IsSiglusEngine() && g_header->lookup_region_offset != 0) {
