...
constexpr uint32_t kTextSourceKirikiriTextRender = 6;
...
// Leaf/AQUAPLUS（WHITE ALBUM2）资源音频三段门的分型位。只记录事实，不参与判定。
...
// 身份门先匹配 exe SHA-256 + machine，再校验唯一锚点、section 角色和返回点。
// 哈希命中位必须在结构门之前置位，才能区分“发行版不符”和“结构复核失败”。
...
// “exe 摘要量不到”是可重试的瞬时条件，不能与“摘要不匹配”共用永久拒绝缓存。
