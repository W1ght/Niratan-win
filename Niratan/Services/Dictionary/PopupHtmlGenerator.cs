...
public sealed class PopupHtmlGenerator
{
        ...
        long documentEpoch = 0)
    {
        ...
        return $@"<!doctype html>
<html lang=""ja"" data-niratan-color-scheme=""{(IsThemeDark(themeMode) ? "dark" : "light")}"" style=""visibility:{(hidden ? "hidden" : "visible")}"">
<head>
<meta charset=""utf-8"" />
...
window.audioRequestEndpoint = 'https://niratan-audio-resolver.local/resolve';
window.customCSS = {JsonSerializer.Serialize(customCss)};
window.dictionaryMediaRequestEndpoint = 'https://niratan-dictionary-media.local/image';
...
document.documentElement.setAttribute('data-niratan-color-scheme', '{(IsThemeDark(themeMode) ? "dark" : "light")}');
...
return window.niratanRedirectResults?.({entriesJson}, {finalResultCount}, {renderGeneration}) === true;
        ...
        long documentEpoch = 0)
    {
        ...
        return $$"""
(() => {
    if (typeof window.niratanAppendResults !== 'function') return 'bridge-missing';
    return window.niratanAppendResults({{entriesJson}}, {{totalResultCount}}, {{documentEpoch}}, {{renderGeneration}})
        ? 'appended'
        : 'stale';
})()
""";
