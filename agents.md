...
```bash
# 构建 x64 Release
dotnet build Hoshi/Hoshi.csproj -p:Platform=x64 -p:RuntimeIdentifier=win-x64

# 构建 x64 Debug（默认）
dotnet build Hoshi/Hoshi.csproj -p:Platform=x64

# 运行全部测试
dotnet test Hoshi.Tests/Hoshi.Tests.csproj -p:Platform=x64

# 运行特定测试
dotnet test Hoshi.Tests/Hoshi.Tests.csproj -p:Platform=x64 --filter "FullyQualifiedName~Dictionary"

# 启动应用（构建后）
.\build-and-run.ps1
```

> `-p:RuntimeIdentifier=win-x64` 用于解决 WindowsAppSDKSelfContained 的架构检测问题，仅在 Release 构建时需要。Debug 构建只需 `-p:Platform=x64`。

### 20.2 测试分类

| 类别 | 说明 | 位置 |
|------|------|------|
| `JapaneseDeinflectorTests` | 动词/形容词变形还原单元测试 | `Hoshi.Tests/Services/Dictionary/` |
| `DictionaryLookupServiceTests` | 字典查询集成测试（需真实字典文件） | `Hoshi.Tests/Services/Dictionary/` |
| `NovelReaderWebAssetTests` | XAML/WebView/JS 资源资产测试（检查源代码模式） | `Hoshi.Tests/Services/Novels/` |
...
1. 构建 x64 Release
2. 运行全部测试确认通过
3. 启动应用，打开小说，手动验证查词弹窗可用

涉及文件清单：
- `Hoshi/Services/Dictionary/JapaneseDeinflector.cs` — 变形还原引擎
- `Hoshi/Services/Dictionary/PopupHtmlGenerator.cs` — 弹窗 HTML/JS 生成
- `Hoshi/Views/Dictionary/DictionaryLookupPopup.cs` — 弹窗 WebView2 控件
- `Hoshi/Views/Dictionary/DictionaryPopupOverlay.cs` — 弹窗栈管理 + 定位
- `Hoshi/Web/DictionaryPopup/popup.js` — 弹窗前端渲染脚本
- `Hoshi/Views/Pages/NovelReaderPage.xaml.cs` — 查词入口

### 20.4 变形还原对齐检查

`JapaneseDeinflector` 已从简单后缀匹配重写为与 Android `hoshidicts/deinflector.cpp` 一致的规则引擎。修改时注意：

- 所有规则在 `InitTransforms()` 中定义，使用 18 种条件标志位链式匹配
- 规则格式：`AddRule(new("后缀", "替换", conditionsIn, conditionsOut, groupId))`
- 特殊动词（`iku_verbs`, `godan_u_special_verbs`, `fu_verb_te_conjugations`）通过 `AddIrregular()` 处理
- 条件标志位定义见 `JapaneseDeinflectionConditions` 枚举，与 Android `Conditions` 枚举一一对应
- 字典词条 rules 匹配使用 `PosToConditions()`，输出 `V1|V5|VK|VS|VZ|ADJ_I` 等复合标志位
- 与 Android 的行为对齐以 `docs/reference/hoshi/Hoshi-Reader-Android/third_party/hoshidicts-kotlin-bridge/app/src/main/cpp/hoshidicts/src/deinflector.cpp` 为参考
- 静态字段初始化顺序：`Instance` 必须通过 static constructor 初始化，确保 `IkuVerbs` 等静态数组在 Instance 构造前已完成初始化

### 20.5 弹窗栈架构（2026-05-20 重构）

```
DictionaryPopupOverlay (栈管理器)
  ├─ Popup (全屏透明 WinUI Popup)
  │   └─ Canvas
  │       ├─ rootHost (DictionaryLookupPopup, 预热复用)
  │       └─ childHosts[0..4] (DictionaryLookupPopup, 嵌套查词)
  └─ 定位: PositionHost() 根据横排/竖排自动选择位置
```

关键设计决策：
- 根弹窗 `WarmAsync()` 预加载 shell HTML，后续查词通过 `ExecuteScriptAsync` 注入数据，避免 WebView2 重新导航
- 子弹窗使用 `NavigateToString()` 完整加载（冷启动，因为数据不同）
- 最大嵌套深度 5，超出时移除最旧子弹窗
- 弹窗关闭（点击外部/按关闭按钮/X钮）一律关闭整个 overlay
- 滚动根弹窗会清除所有子弹窗
- 弹窗定位根据 `isVertical` 参数自动选择：竖排优先左右，横排优先上下
- 弹窗内嵌套查词：点击释义中的日语文本 → popup.js 发送 `lookupRedirect` → C# 创建子弹窗
