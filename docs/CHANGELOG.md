...
**顶部操作收敛**：首页和发现不再显示空占位命令行，系列与全部视频只保留搜索、排序。导入页顶部按 Fushi 主动作样式显示带图标和文字的“扫描文件夹 / YouTube / 刷新 / 重新刮削 / 后台任务”；后台任务详情只在导入内容区原位展开，切换到其他 Video 页签时立即关闭，不再覆盖系列或全部视频。来源设置、打开文件夹和移除等单来源操作继续放在来源卡片内，避免所有页面重复显示一排无关按钮。
...
### Google Video 签名流可以解析但无法播放

**原因**：当前 Google Video CDN 接受带明确结束位置的字节范围，却会对 FFmpeg/libmpv 默认发出的开放式 `Range: bytes=n-` 返回 HTTP 403。解析和入库因此都能成功，但 libmpv 在真正打开媒体时以 `MPV_ERROR_LOADING_FAILED` 结束并显示 `No file`；User-Agent 中未转义的逗号还会被 mpv 列表属性截断。

**解决**：在 libmpv 播放边界注册只读流 callback，只对带可信 `clen` 的 HTTPS `*.googlevideo.com` URL 生效。桥接层把视频和外部音频分别转换为不透明内存 URI，以 1 MiB 有限 Range 分块读取并校验 `206 Content-Range`，同时保留 seek/size/cancel 语义；普通远程源继续直连。mpv 字符串请求头同步转义列表分隔符，签名 URL、headers 和 token 均不落盘或写入日志。

### 长字幕自动折行后显示不全

**原因**：字幕面板高度只按 SRT 文本里的显式换行计算，没有考虑 Canvas 在当前视频宽度内产生的自动折行；YouTube 自动字幕通常把很长的 cue 保存成单行，大字号和右侧检查器同时开启时，第三行以后会超出固定面板而被裁掉。

**解决**：字幕布局现在使用视频 viewport 的实际宽度估算中日文、拉丁字符和显式换行所需行数，动态增加面板高度并预留阴影/模糊空间；窗口或检查器宽度变化时立即重排。只有极端 cue 超过视频高度时才临时降低该条的有效字号，不改全局字号设置；Canvas 与隐藏选词层共享有效字号，避免显示修复后查词坐标偏移。
