...
**Architecture:** Keep the existing dashboard view model, adaptive placement, and data templates unchanged. Express both visual requirements as XAML layout contracts: a fixed `118×118` ring host and a top-aligned week card whose height is content-driven inside a shared Grid row.
...
- Produces: a Today ring host with exact effective size `118×118` and a `WeekCard` with `VerticalAlignment="Top"`.
...
Expected: FAIL because the Today card still contains `Width="88"` and the Week card has no `VerticalAlignment="Top"`.

- [ ] **Step 3: Implement the minimal XAML changes**

Change the Today ring host to:
...
**原因**：
- 今日目标圆环仍使用 88×88 的 Windows 初始尺寸，明显小于 Hoshi 对应的 118×118 目标环。
- 宽屏三列布局中，本周卡片与更高的排行卡共用同一 Grid 行；Border 默认纵向拉伸，使本周卡片内部出现大块空白。

**解决**：
- 将今日目标圆环固定为 118×118 effective pixels，对齐 Hoshi 的视觉层级。
