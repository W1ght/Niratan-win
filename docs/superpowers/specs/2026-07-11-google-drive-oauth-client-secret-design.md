...
Hoshi Windows 使用桌面 OAuth 的 loopback 回调和 PKCE。Google 已能把授权码回传到 `127.0.0.1`，但当前 token 请求只发送 `client_id`，实际客户端返回 `client_secret is missing`。同时，本地回调页在 token 交换完成前就显示 “Google Drive connected”，会把“收到授权码”误报成“连接完成”。
...
loopback 接收器在收到无 OAuth 错误的回调时显示中性文案，例如 “Google Drive authorization received. Return to Hoshi to finish connecting.”。只有 Hoshi 完成 token 交换并保存凭据后，应用设置页才显示 `Connected`。
