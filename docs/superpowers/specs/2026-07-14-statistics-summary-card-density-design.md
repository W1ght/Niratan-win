...
Align the Windows statistics summary cards with the supplied Niratan reference:
...
The Today goal ring was hosted in an `88×88` grid. More importantly, WinUI's default `ProgressRing` style sets the control itself to `32×32` and centers it. Enlarging only the host therefore leaves the rendered ring at its default size. The Niratan Mac statistics dashboard uses a `118×118` frame for the equivalent ring.
...
1. Change both the Today ring host and the determinate `ProgressRing` itself to `118×118` effective pixels, matching the Niratan Mac reference implementation. Keep the percentage binding, metrics, and accessibility behavior.
...
- Launch Niratan and use screenshot-only UI verification at narrow and wide widths. Confirm the ring is visibly larger, the week card has no stretched empty interior, and the dashboard remains responsive.
