...
- let the user move the selected date window with a horizontal scrollbar;
...
The anchor-date picker and the public `AnchorDate` ViewModel state are removed. A horizontal WinUI `ScrollBar` below the chart selects one discrete, calendar-aligned window from the available Dashboard data window.
...
- The scrollbar value is always an integer period index.
- `Year` has one period, so the scrollbar remains visible for layout stability but is disabled.
...
- Selecting a day in the reading calendar moves the scrollbar to the period containing that day, then updates the day-detail card.

The selected period continues to filter the entire Dashboard, including the trend, selected-range summary, speed summary, book ranking, shelf comparison, and calendar selection styling. The scrollbar is not a chart-only visual pan and never mutates source statistics.
...
- scrollbar: placed below the chart by the Dashboard view and not counted in the 260-pixel chart height.
...
- scrollbar minimum, maximum, viewport size, small change, and large change;
...
The range mode, metric, grain, and style controls remain. The anchor-date label and `CalendarDatePicker` are removed. The new scrollbar appears immediately under the fixed-height chart with an automation identifier and localized accessible name.

## Accessibility and Localization

- The scrollbar exposes its selected range through its accessible name/help text.
...
- Empty snapshots produce no selectable periods and a disabled scrollbar.
...
- the XAML contract has a fixed 260-pixel chart, a horizontal scrollbar, and no anchor-date picker;
...
- disabled year scrollbar behavior;
...
- persisting the scrollbar position across app restarts;
