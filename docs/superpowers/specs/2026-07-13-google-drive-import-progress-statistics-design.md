...
Niratan 从 Google Drive 下载 TTU 书籍后，存在两个用户可见问题：
...
- 运行 `dotnet test Niratan.Tests/Niratan.Tests.csproj -c Debug -p:Platform=x64`。
- 构建并启动 Niratan，使用本地假 Drive 数据或受控测试 fixture 验证下载流程，不访问真实 Google Drive。
