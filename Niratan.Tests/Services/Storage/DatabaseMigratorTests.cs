...
public class DatabaseMigratorTests
{
    ...
    {
        var ct = TestContext.Current.CancellationToken;
        var dbPath = Path.Combine(Path.GetTempPath(), $"niratan-migration-v8-{Guid.NewGuid():N}.db");
    ...
    {
        var ct = TestContext.Current.CancellationToken;
        var dbPath = Path.Combine(Path.GetTempPath(), $"niratan-migration-v11-{Guid.NewGuid():N}.db");
    ...
    {
        var ct = TestContext.Current.CancellationToken;
        var dbPath = Path.Combine(Path.GetTempPath(), $"niratan-video-only-{Guid.NewGuid():N}.db");
        var connectionString = $"Data Source={dbPath};Pooling=False";
        try
        {
    ...
    )
    {
        ...
        var migrationType = appAssembly.GetType(
            $"Niratan.Services.Storage.Migrations.{migrationName}",
            throwOnError: true
        )!;
