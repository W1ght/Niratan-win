...
public class NovelReaderWebAssetTests
{
    ...
    [Fact]
    public void ReaderBridge_KeepsNiratanReaderApiInsideClosure()
    {
        ...
        script.Should().NotContain("window.niratanReader");
    ...
    [Fact]
    public void ReaderBridge_UsesNiratanStyleChapterMessages()
    {
        var script = File.ReadAllText(Path.Combine(ReaderRoot, "reader-bridge.js"));
    ...
    public void ReaderBridge_ResolvesTypedChapterEndpointsOnceBeforeChapterReady()
    {
        ...
        script.Should().Contain("window.__niratanChapterInfo.restoreTarget ?? null");
    ...
    public void ReaderBridge_TerminalEventsCarryGenerationChapterAndAttemptIdentity()
    {
        ...
        script.Should().Contain("renderAttemptId: window.__niratanChapterInfo.renderAttemptId");
    ...
    public void ReaderBridge_SequentialOperationsEachGetOneShotFailureReporter()
    {
        ...
        var autoInitializeBody = Regex.Match(
            script,
            @"(?s)if \(window\.__niratanChapterInfo\).*?\n\} else \{").Value;
        ...
        script.Should().NotContain("window.niratanBridgeErrorReported");
    ...
    public void ReaderBridge_HandlesErrorWithDetails()
    {
        ...
        script.Should().Contain("window.__niratanReaderState.error");
    }
    ...
    public void ReaderBridge_NavigatesBetweenPagesAndChapters()
    {
        ...
        script.Should().NotContain("window.niratanReaderNavigateAuthorized");
        ...
        script.Should().Contain("window.__niratanReaderShortcutBindings");
        ...
        pageCode.Should().Contain("window.__niratanReaderShortcutBindings");
    ...
    public void NovelPages_DefineStableAutomationIdsForUiAutomation()
    {
        ...
        libraryXaml
            .Should()
            .Contain("AutomationId=\"{x:Bind AutomationId");
        readerXaml
            .Should()
    ...
    public void DictionaryPopup_UsesDesktopReferenceDictionaryCards()
    {
        ...
        popupCss.Should().MatchRegex(
            @"(?s)html\[data-niratan-color-scheme=""light""\],\s*html\[data-niratan-color-scheme=""light""\] body\s*\{[^}]*--popup-card-border-color:\s*rgba\(0, 0, 0, 0\.14\);[^}]*--popup-card-inner-highlight:\s*rgba\(255, 255, 255, 0\.34\);[^}]*--popup-card-shadow-color:\s*rgba\(0, 0, 0, 0\.10\);[^}]*\}");
    }
    ...
    [Fact]
    public void DictionaryPopupHistory_ResetIsScopedToPendingRootReplacement()
    {
        var popupJs = File.ReadAllText(Path.Combine(
            ProjectRoot, "Web", "DictionaryPopup", "popup.js"));

        var promoteStart = popupJs.IndexOf("function promoteFirstFrame()", StringComparison.Ordinal);
        var promoteEnd = popupJs.IndexOf("function renderAvailableEntries()", promoteStart, StringComparison.Ordinal);
        var promote = popupJs[promoteStart..promoteEnd];
        var pendingStart = promote.IndexOf("if (pendingPayload)", StringComparison.Ordinal);
        var committedAssignment = promote.IndexOf(
            "window.popupRenderGeneration = generation;",
            pendingStart,
            StringComparison.Ordinal);
        var pendingRootPath = promote[pendingStart..committedAssignment];
        var sharedPromotionPath = promote[committedAssignment..];

        pendingRootPath.Should().Contain("backStack.length = 0;");
        ...
        var redirectStart = popupJs.IndexOf(
            ...
            StringComparison.Ordinal);
        var redirectEnd = popupJs.IndexOf(
            "window.niratanAppendResults = function",
            redirectStart,
            StringComparison.Ordinal);
        var redirect = popupJs[redirectStart..redirectEnd];
        var redirectPush = redirect.IndexOf("backStack.push(snapshot());", StringComparison.Ordinal);
        var redirectForwardReset = redirect.IndexOf("forwardStack.length = 0;", StringComparison.Ordinal);
        var redirectRender = redirect.IndexOf("window.renderPopup();", StringComparison.Ordinal);
        redirectPush.Should().BeLessThan(redirectForwardReset);
        redirectForwardReset.Should().BeLessThan(redirectRender);
        redirect.Should().NotContain("backStack.length = 0;");

        var replaceStart = popupJs.IndexOf(
            "window.replacePopupResults = function",
            StringComparison.Ordinal);
        var replaceEnd = popupJs.IndexOf("function capturePopupRuntime()", replaceStart, StringComparison.Ordinal);
        var replace = popupJs[replaceStart..replaceEnd];
        replace.IndexOf("backStack.length = 0;", StringComparison.Ordinal)
            .Should().BeLessThan(replace.IndexOf("window.renderPopup();", StringComparison.Ordinal));
        replace.IndexOf("forwardStack.length = 0;", StringComparison.Ordinal)
            .Should().BeLessThan(replace.IndexOf("window.renderPopup();", StringComparison.Ordinal));
    }

    [Fact]
    public void DictionaryPopupNavigationState_UsesExactCommittedEpochAndGeneration()
    {
        var popupJs = File.ReadAllText(Path.Combine(
            ProjectRoot, "Web", "DictionaryPopup", "popup.js"));
        var popupCode = File.ReadAllText(Path.Combine(
        ...
        var postStart = popupJs.IndexOf("function postNavigationState()", StringComparison.Ordinal);
        var postEnd = popupJs.IndexOf("function appendPendingHistoryRestore", postStart, StringComparison.Ordinal);
        var post = popupJs[postStart..postEnd];
        post.Should().Contain("epoch: window.niratanPopupDocumentEpoch");

        var updateStart = popupCode.IndexOf("private void UpdateNavigationState", StringComparison.Ordinal);
        var updateEnd = popupCode.IndexOf("private static DictionaryPopupRedirectRequest", updateStart, StringComparison.Ordinal);
        var update = popupCode[updateStart..updateEnd];
        update.Should().Contain("TryGetContentEpoch(payload, out var epoch)");
        update.Should().Contain("_displayTransaction.CommittedGeneration");
        update.Should().Contain("DictionaryPopupDocumentEpoch.Matches(_rendererEpoch, epoch)");
        update.Should().Contain("IsCurrent(epoch, generation)");
        update.Should().NotContain("_displayGeneration");
    }

    [Fact]
    public void DictionaryPopupActionBar_SeparatesVisibilityFromNavigationReset()
    {
        var popupCode = File.ReadAllText(Path.Combine(
            ProjectRoot, "Views", "Dictionary", "DictionaryLookupPopup.cs"));

        popupCode.Should().Contain("private void UpdateActionBarVisibility(");
        popupCode.Should().Contain("private void ResetActionBarNavigationState()");

        var redirectStart = popupCode.IndexOf(
            "public async Task<bool> ShowRedirectResultsAsync(",
            StringComparison.Ordinal);
        var redirectEnd = popupCode.IndexOf("public async Task NavigateBackAsync", redirectStart, StringComparison.Ordinal);
        var redirect = popupCode[redirectStart..redirectEnd];
        redirect.Should().Contain("UpdateActionBarVisibility(displaySettings)");
        redirect.Should().NotContain("ResetActionBarNavigationState()");

        var promoteStart = popupCode.IndexOf("private void PromoteNativeContext(", StringComparison.Ordinal);
        var promoteEnd = popupCode.IndexOf("private async Task ProcessQueuedShowAsync", promoteStart, StringComparison.Ordinal);
        var promote = popupCode[promoteStart..promoteEnd];
        promote.Should().Contain("ResetActionBarNavigationState()");

        var hideStart = popupCode.IndexOf("public void Hide()", StringComparison.Ordinal);
        var hideEnd = popupCode.IndexOf("public bool CancelPendingContent", hideStart, StringComparison.Ordinal);
        var hide = popupCode[hideStart..hideEnd];
        hide.Should().Contain("ResetActionBarNavigationState()");
    }
    ...
    public void SettingsPage_UsesVisibilityConvertersForBooleanVisibilityBindings()
    {
        ...
        settingsXaml.Should().Contain("Tag=\"Niratan.Views.Pages.ReaderAppearanceSettingsPage\"");
        ...
        bridgeScript.Should().Contain("niratanWheelNavigationEnabled");
    ...
    public void SettingsPage_ExposesMacAlignedAdvancedSettingsHome()
    {
        ...
        advancedXaml.Should().Contain("x:Class=\"Niratan.Views.Pages.AdvancedSettingsPage\"");
    ...
    public void SettingsPage_ExposesMacAlignedSasayakiSettings()
    {
        ...
        sasayakiXaml.Should().Contain("x:Class=\"Niratan.Views.Pages.SasayakiSettingsPage\"");
    ...
    public void SettingsPage_ExposesMacAlignedStatisticsSettings()
    {
        ...
        statisticsXaml.Should().Contain("x:Class=\"Niratan.Views.Pages.StatisticsSettingsPage\"");
    ...
    public void SelectionScript_HighlightsMatchedLookupTextInReader()
    {
        ...
        script.Should().Contain("CSS.highlights?.set('niratan-selection', new Highlight(...highlights))");
        script.Should().Contain("CSS.highlights?.get('niratan-selection')?.clear()");
        readerCode.Should().Contain("HighlightLookupSelectionAsync(results[0].Matched)");
        readerCode.Should().Contain("window.niratanSelection.highlightSelection");
        stylesCode.Should().Contain("::highlight(niratan-selection)");
    ...
    public void NavigationPage_ExposesNovelLookupPage()
    {
        ...
        navigationXaml.Should().Contain("Tag=\"Niratan.Views.Pages.NovelLookupPage\"");
    ...
    public void GlobalLookupWindow_UsesSharedPopupOverlayAndNoCustomResultsList()
    {
        ...
        windowXaml.Should().Contain("x:Class=\"Niratan.Views.Dictionary.GlobalLookupWindow\"");
    ...
    {
        var projectCode = File.ReadAllText(Path.Combine(ProjectRoot, "Niratan.csproj"));
        var appCode = File.ReadAllText(Path.Combine(ProjectRoot, "App.xaml.cs"));
        var appSettingsCode = File.ReadAllText(
            Path.Combine(ProjectRoot, "Models", "Settings", "AppSettings.cs")
        );
    ...
    [Fact]
    public void DictionaryPopupOverlay_UsesNiratanPopupPlacement()
    {
        var overlayCode = File.ReadAllText(
            Path.Combine(ProjectRoot, "Views", "Dictionary", "DictionaryPopupOverlay.cs")
        );
        ...
        overlayCode.Should().Contain("aligned with Niratan Reader Mac PopupLayout");
        overlayCode.Should().Contain("DictionaryPopupLayoutCalculator.Resolve");
        ...
        popupJs.Should().MatchRegex(
            @"postPopupMessage\('contentPrepared',\s*\{\s*epoch: window\.niratanPopupDocumentEpoch,\s*generation: generation\s*\}\)");
        popupJs.Should().MatchRegex(
            @"postPopupMessage\('contentReady',\s*\{\s*epoch: window\.niratanPopupDocumentEpoch,\s*generation: generation\s*\}\)");
    ...
    public void DictionaryLookupPopup_PreservesCommittedVisualDuringReplacement()
    {
        ...
        var showEnd = code.IndexOf("public async Task<bool> ShowRedirectResultsAsync(", showStart, StringComparison.Ordinal);
    ...
    public void DictionaryPopupOverlay_AnchorsChildPopupToParentSelectionRect()
    {
        ...
        popupCode.Should().Contain("window.niratanHighlightPopupSelection?.(");
        ...
        popupGenerator.Should().Contain("CSS.highlights?.set('niratan-selection', new Highlight(...highlights))");
    }
    ...
    public void DictionaryPopup_StagesReplacementUntilFirstFrameReady()
    {
        ...
        js.Should().Contain("window.niratanStagePopupRender = function (pendingPayload)");
        ...
        js.Should().MatchRegex(
            @"postPopupMessage\('contentPrepared',\s*\{\s*epoch: window\.niratanPopupDocumentEpoch,\s*generation: generation\s*\}\)");
        js.Should().Contain("window.niratanCommitPopupRender = function (expectedEpoch, expectedGeneration)");
        js.Should().Contain("liveContainer.replaceChildren.apply(liveContainer");
        js.Should().Contain("window.niratanCancelPopupRender = function (expectedEpoch, expectedGeneration)");
        js.Should().Contain("window.niratanGetCommittedPopupGeneration = function (expectedEpoch)");
        js.Should().Contain("window.niratanDiscardPopupRender = function (expectedEpoch, expectedGeneration)");
        js.Should().Contain("window.niratanPendingPopupRender");
        js.Should().Contain("function attachPopupInteractionHandlers()");

        var stageStart = js.IndexOf("window.niratanStagePopupRender = function", StringComparison.Ordinal);
        var commitStart = js.IndexOf("window.niratanCommitPopupRender = function", stageStart, StringComparison.Ordinal);
        ...
        var stageBranchStart = injection.IndexOf("if (stageRender)", StringComparison.Ordinal);
        var stageBranchEnd = injection.IndexOf("\n        return $@\"", stageBranchStart, StringComparison.Ordinal);
        var stageInjection = injection[stageBranchStart..stageBranchEnd];
        stageInjection.Should().NotContain("document.documentElement.setAttribute");
        stageInjection.Should().NotContain("document.documentElement.style.setProperty");
        stageInjection.Should().NotContain("window.dictionaryStyles =");
        stageInjection.Should().NotContain("window.lookupEntries =");
        stageInjection.Should().NotContain("window.audioSources =");
        stageInjection.Should().NotContain("window.useAnkiConnect =");
    ...
    public void DictionaryLookupAudioPath_EmitsEndToEndLatencyTraceLogs()
    {
        ...
        popupCode.Should().Contain("\"https://niratan-audio-resolver.local/*\"");
    ...
    public void DictionaryPopupOverlay_ReusesChildWebViewHostsDuringRedirectStorms()
    {
        ...
        overlayCode.Should().Contain("Math.Min(context.DisplaySettings.MaxResults, NestedLookupMaxResults)");
    ...
    public void NovelBookCards_AreInvokableAutomationTargets()
    {
        ...
        libraryXaml.Should().Contain("<controls:NovelBookCard");
        cardXaml.Should().Contain("<Button");
        cardXaml.Should().Contain(
            "AutomationProperties.AutomationId=\"{Binding AutomationId, ElementName=Root}\"");
    ...
    [Fact]
    public void ReaderPage_HostsSasayakiResourcesAndLibraryHasNoMatchAction()
    {
        var libraryXaml = File.ReadAllText(
            Path.Combine(ProjectRoot, "Views", "Pages", "NovelLibraryPage.xaml")
        );
        ...
        var resourcesViewModel = File.ReadAllText(
            Path.Combine(ProjectRoot, "ViewModels", "Components", "SasayakiResourcesViewModel.cs")
        );
        var appCode = File.ReadAllText(Path.Combine(ProjectRoot, "App.xaml.cs"));
        var enResources = File.ReadAllText(
            Path.Combine(ProjectRoot, "Strings", "en-US", "Resources.resw")
        ...
        libraryXaml.Should().NotContain("NovelBookMatchSasayakiMenuItem");
        libraryCode.Should().NotContain("MatchSasayakiMenuItem_Click");
    ...
    public void ReaderBridge_ExposesReaderDiagnosticsForAutomation()
    {
        ...
        script.Should().Contain("window.__niratanReaderState");
        ...
        script.Should().Contain("window.niratanGetDiagnostics");
    }

    [Fact]
    public void ReaderBridge_RecordsNiratanStyleLayoutMetrics()
    {
        var script = File.ReadAllText(Path.Combine(ReaderRoot, "reader-bridge.js"));
    ...
    public void ReaderContentHelpers_DoNotDependOnPrivatePaginationEngine()
    {
        ...
        bridge.Should().NotContain("window.niratanReader");
        selection.Should().NotContain("window.niratanReader");
        highlights.Should().NotContain("window.niratanReader");
    }
    ...
    public void ReaderPage_CapturesWebViewPreviewWhenArtifactDirectoryIsConfigured()
    {
        ...
        pageCode.Should().Contain("NIRATAN_NOVEL_READER_ARTIFACT_DIR");
    ...
    [Fact]
    public void ReaderPage_CreatesAndLoadsPersistedHighlightsThroughNativeContextMenu()
    {
        var readerCode = File.ReadAllText(
            Path.Combine(ProjectRoot, "Views", "Pages", "NovelReaderPage.xaml.cs")
        );
        ...
        highlightsScript.Should().Contain("window.niratanHighlights");
        highlightsScript.Should().Contain("applyHighlights");
        highlightsScript.Should().Contain("createHighlight(color, id)");
        highlightsScript.Should().Contain("selectionBoundaryOffset");
        highlightsScript.Should().Contain("fragment.querySelectorAll?.(\"rt, rp\")");
        highlightsScript.Should().Contain("removeHighlight");
        highlightsScript.Should().Contain("collectSegments");
        highlightsScript.Should().Contain("niratan-highlight-yellow");
        ...
        readerCode.Should().Contain("ContextMenuRequested += OnReaderContextMenuRequested");
        readerCode.Should().Contain("CoreWebView2ContextMenuItemKind.Submenu");
        readerCode.Should().Contain("CreateHighlightFromSelectionAsync");
        readerCode.Should().Contain("ViewModel.AddHighlightAsync");
        readerCode.Should().Contain("ViewModel.GetChapterHighlightsJson(destinationChapterIndex)");
        readerCode.Should().Contain("window.__niratanChapterHighlights");
        bridgeScript.Should().Contain("window.niratanHighlights.applyHighlights(window.__niratanChapterHighlights || [])");
        appCode.Should().Contain("IReaderHighlightService, ReaderHighlightService");

        var viewModelCode = File.ReadAllText(
            Path.Combine(ProjectRoot, "ViewModels", "Pages", "NovelReaderPageViewModel.cs")
        );
        viewModelCode.Should().Contain("AddHighlightAsync");
        viewModelCode.Should().Contain("CreateFromChapterSelection");
        ...
        foreach (var key in new[]
        {
            "ReaderHighlightContextMenuLabel",
            "ReaderHighlightColorYellow",
            "ReaderHighlightColorGreen",
            "ReaderHighlightColorBlue",
            "ReaderHighlightColorPink",
            "ReaderHighlightColorPurple",
        })
        {
            enResources.Should().Contain(key);
            zhResources.Should().Contain(key);
    ...
    public void ReaderAppearance_ExposesMacAlignedStatisticsChromeSettings()
    {
        ...
        readerXaml.Should().Contain("x:Name=\"NovelReaderStatisticsToggleButton\"");
        readerXaml.Should().Contain("Click=\"StatisticsToggleButton_Click\"");
        readerXaml.Should().Contain("<StackPanel Grid.Column=\"0\"");
        readerXaml.Should().Contain("<StackPanel Grid.Column=\"2\"");
        readerCode.Should().Contain("ReaderBottomChrome.Background = new SolidColorBrush(backgroundColor)");
        ...
        readerCode.Should().Contain("NovelReaderStatisticsText.Visibility == Visibility.Visible");
        readerCode.Should().Contain("NovelReaderStatisticsToggleButton.Visibility == Visibility.Visible");

        var normalizedReaderCode = readerCode.ReplaceLineEndings("\n");
        var visibilityMethodStart = normalizedReaderCode.IndexOf(
            "private void UpdateStatisticsButtonVisibility()",
            StringComparison.Ordinal);
        var visibilityMethodEnd = normalizedReaderCode.IndexOf(
            "private void ToggleReaderFocusMode()",
            visibilityMethodStart,
            StringComparison.Ordinal);
        var visibilityMethod = normalizedReaderCode[visibilityMethodStart..visibilityMethodEnd];

        visibilityMethod.Should().Contain(
            "NovelReaderStatisticsButton.Visibility = CurrentStatisticsSettings.EnableStatistics\n            ? Visibility.Visible");
        visibilityMethod.Should().Contain(
            "NovelReaderStatisticsToggleButton.Visibility = CurrentStatisticsSettings.EnableStatistics\n            && readerSettings.Current.ShowStatisticsToggle");
    ...
    public void ReaderPage_ExposesLocalizedSasayakiControlsForAutomation()
    {
        ...
        foreach (var key in new[]
        {
            ...
            "NovelReaderSasayakiResourcesTab.Content",
            "NovelReaderSasayakiChaptersTab.Content",
            "NovelReaderSasayakiSettingsTab.Content",
            "NovelReaderSasayakiNoChaptersText.Text",
            "NovelReaderSasayakiCurrentChapterText.Text",
    ...
    public void ReaderPage_DefinesNiratanStyleSasayakiPanelAndLyricsMode()
    {
        ...
        foreach (var requiredToken in new[]
        {
            ...
            "NovelReaderSasayakiPlaybackHeader",
            "NovelReaderSasayakiTabView",
            "NovelReaderSasayakiAudiobookResourceCard",
            "NovelReaderSasayakiSubtitleMatchCard",
            "NovelReaderSasayakiChapterList",
            ...
            "NovelReaderSasayakiOpenAudiobookButton",
            "NovelReaderSasayakiOpenSubtitleButton",
            "NovelReaderSasayakiMatchButton",
    ...
    [Fact]
    public void ReaderLyricsMode_BlankClickDismissesLookupWithoutMakingHoverDestructive()
    {
        var lyricsXaml = File.ReadAllText(Path.Combine(
            ProjectRoot,
            "Views",
            "Controls",
            "ReaderLyricsModeControl.xaml"));
        var lyricsCode = File.ReadAllText(Path.Combine(
            ProjectRoot,
            "Views",
            "Controls",
            "ReaderLyricsModeControl.xaml.cs"));
        var readerCode = File.ReadAllText(Path.Combine(
            ProjectRoot,
            "Views",
            "Pages",
            "NovelReaderPage.xaml.cs"));

        lyricsXaml.Should().Contain("PointerPressed=\"LyricsRoot_PointerPressed\"");
        lyricsCode.Should().Contain("if (!isHoverLookup)");
        lyricsCode.Should().Contain("DismissLookupRequested?.Invoke(this, EventArgs.Empty)");
        readerCode.Should().Contain("ReaderLyricsMode.DismissLookupRequested +=");
        readerCode.Should().Contain("_popupOverlay?.Dismiss();");
    ...
    [Fact]
    public void ReaderLyricsMode_StatisticsIconReflectsTrackingState()
    {
        var lyricsXaml = File.ReadAllText(Path.Combine(
            ProjectRoot,
            "Views",
        ...
        lyricsXaml.Should().Contain("x:Name=\"LyricsStatisticsIcon\"");
        lyricsCode.Should().Contain("e.PropertyName == nameof(ViewModel.IsStatisticsTracking)");
        lyricsCode.Should().Contain("LyricsStatisticsIcon.Glyph = ViewModel.IsStatisticsTracking");
        lyricsCode.Should().Contain("? \"\\uE823\"");
        lyricsCode.Should().Contain(": \"\\uE9D2\"");
    }

    [Fact]
    public void ReaderLyricsMode_UsesNiratanPlayerLayoutAndFitsTheWholeCover()
    {
        var lyricsXaml = File.ReadAllText(Path.Combine(
            ProjectRoot,
            "Views",
        ...
        lyricsXaml.Should().Contain("<Viewbox MaxHeight=\"430\"");
        lyricsXaml.Should().MatchRegex(
            "(?s)x:Name=\\\"LyricsCoverImage\\\".*?Stretch=\\\"Uniform\\\"");
        lyricsXaml.Should().NotMatchRegex(
            "(?s)x:Name=\\\"LyricsCoverImage\\\".*?Stretch=\\\"UniformToFill\\\"");
        lyricsXaml.Should().NotContain("ViewModel.CurrentCueText");
        lyricsXaml.Should().Contain("Height=\"8\"");
        lyricsXaml.Should().Contain("x:Name=\"LyricsMaskIcon\"");
        lyricsXaml.Should().Contain("x:Name=\"VerticalLyricsIcon\"");
        lyricsXaml.Should().MatchRegex(
            "(?s)x:Name=\\\"LyricsExitButton\\\".*?Canvas.ZIndex=\\\"10\\\".*?Margin=\\\"0,36,52,0\\\"");
    }

    [Fact]
    public void ReaderLyricsMode_RemovesNovelWebViewFromCompositionUntilExit()
    {
        var readerCode = File.ReadAllText(Path.Combine(
            ProjectRoot,
            "Views",
        ...
        readerCode.Should().Contain("SetNovelReaderSurfaceSuppressedForLyrics(suppress: true);");
        readerCode.Should().Contain("SetNovelReaderSurfaceSuppressedForLyrics(suppress: false);");
        readerCode.Should().Contain(
            "NovelWebView.Visibility = suppress ? Visibility.Collapsed : Visibility.Visible;");
        readerCode.Should().Contain("NovelWebView.IsHitTestVisible = !suppress;");
    }

    [Fact]
    public void ReaderLyricsMode_OwnsShortcutsAndSuppressesDuplicateKeyRoutes()
    {
        var lyricsXaml = File.ReadAllText(Path.Combine(
            ProjectRoot,
            "Views",
        ...
        var readerCode = File.ReadAllText(Path.Combine(
            ...
            "Pages",
            "NovelReaderPage.xaml.cs"));
        var readerBridge = File.ReadAllText(Path.Combine(
            ProjectRoot,
            "Web",
            "NovelReader",
            "reader-bridge.js"));

        lyricsXaml.Should().Contain("IsTabStop=\"True\"");
        readerCode.Should().Contain("ReaderLyricsMode.Focus(FocusState.Programmatic)");
        readerCode.Should().Contain("SuspendReaderKeyboardAccelerators();");
        readerCode.Should().Contain("RegisterReaderKeyboardAccelerators();");
        readerCode.Should().Contain("args.KeyStatus.WasKeyDown");
        readerCode.Should().Contain("IsConfiguredReaderShortcutBinding(binding)");
        readerCode.Should().Contain("_suppressNextLyricsCharacterReceived");
        readerBridge.Should().Contain("event.defaultPrevented || event.repeat");

        var webShortcutCase = readerCode.IndexOf("case \"shortcut\":", StringComparison.Ordinal);
        var lyricsModeGuard = readerCode.IndexOf(
            "if (_isReaderLyricsMode)",
            webShortcutCase,
            StringComparison.Ordinal);
        var webShortcutPayload = readerCode.IndexOf(
            "var shortcutPayload",
            webShortcutCase,
            StringComparison.Ordinal);
        webShortcutCase.Should().BeGreaterThanOrEqualTo(0);
        lyricsModeGuard.Should().BeGreaterThan(webShortcutCase);
        webShortcutPayload.Should().BeGreaterThan(lyricsModeGuard);

        var shortcutsChangedStart = readerCode.IndexOf(
            "private void OnReaderShortcutsChanged",
            StringComparison.Ordinal);
        var lyricsModeRegistrationGuard = readerCode.IndexOf(
            "if (!_isReaderLyricsMode)",
            shortcutsChangedStart,
            StringComparison.Ordinal);
        var shortcutLabelsRefresh = readerCode.IndexOf(
            "ApplyReaderShortcutLabels();",
            shortcutsChangedStart,
            StringComparison.Ordinal);
        shortcutsChangedStart.Should().BeGreaterThanOrEqualTo(0);
        lyricsModeRegistrationGuard.Should().BeGreaterThan(shortcutsChangedStart);
        shortcutLabelsRefresh.Should().BeGreaterThan(lyricsModeRegistrationGuard);

        var keyDownStart = readerCode.IndexOf(
            "private async void NovelReaderPage_KeyDown",
            StringComparison.Ordinal);
        var fallbackClaim = readerCode.IndexOf(
            "_lastKeyDownShortcutBinding = binding;",
            keyDownStart,
            StringComparison.Ordinal);
        var fallbackDispatch = readerCode.IndexOf(
            "args.Handled = await TryHandleReaderShortcutBindingAsync(binding);",
            keyDownStart,
            StringComparison.Ordinal);
        fallbackClaim.Should().BeGreaterThan(keyDownStart);
        fallbackDispatch.Should().BeGreaterThan(fallbackClaim);
    }
    ...
    [Fact]
    public void ReaderPage_UsesCompactGoToAndSasayakiWithWideAppearancePanel()
    {
        var readerXaml = File.ReadAllText(
            Path.Combine(ProjectRoot, "Views", "Pages", "NovelReaderPage.xaml")
        );
    ...
    public void ReaderPage_AppliesSasayakiSettingsToPlaybackUiAndHighlighting()
    {
        ...
        readerCode.Should().NotContain("window.niratanSasayaki");
        readerCode.Should().Contain("settings.DarkBackgroundColor");
        bridgeScript.Should().Contain("setColors: function");
        bridgeScript.Should().Contain("--niratan-sasayaki-background-color");
    ...
    public void ReaderSasayakiHighlight_UsesContinuousRangeWithoutActiveOutline()
    {
        ...
        bridgeScript.Should().Contain("CSS.highlights.set(\"niratan-sasayaki\"");
        bridgeScript.Should().Contain("new Highlight(...ranges)");
        bridgeScript.Should().Contain("window.niratanSelection?.clearSelection?.()");
        ...
        bridgeScript.Should().NotContain("niratan-sasayaki-highlight-active");
    ...
    public void DictionaryPopupOverlay_AtomicallyPromotesPendingRootStateByGeneration()
    {
        ...
        code.Should().Contain("var committedLayout = ResolveCommittedRootLayout(committed.Layout)");
        code.Should().Contain("ApplyHostLayout(_rootHost, committedLayout)");
    ...
    [Fact]
    public void DictionaryPopupOverlay_UsesVisibleEmbeddedPanelBoundsWhenRootContentCommits()
    {
        var code = File.ReadAllText(
            Path.Combine(ProjectRoot, "Views", "Dictionary", "DictionaryPopupOverlay.cs"));

        var commitHandlerStart = code.IndexOf(
            "private void OnRootContentCommitted(",
            StringComparison.Ordinal);
        var abortHandlerStart = code.IndexOf(
            "private void OnRootContentCommitAborted(",
            commitHandlerStart,
            StringComparison.Ordinal);

        commitHandlerStart.Should().BeGreaterThanOrEqualTo(0);
        abortHandlerStart.Should().BeGreaterThan(commitHandlerStart);

        var commitHandler = code[commitHandlerStart..abortHandlerStart];
        commitHandler.Should().Contain("ResolveCommittedRootLayout(committed.Layout)");
        commitHandler.Should().Contain("ApplyHostLayout(_rootHost, committedLayout)");
        code.Should().Contain("_embeddedPanel.ActualWidth");
        code.Should().Contain("_embeddedPanel.ActualHeight");
    }
