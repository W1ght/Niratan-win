...
const document = {
  ...
  readyState: "complete",
  baseURI: "https://niratan-novel-book.local/Text/chapter.xhtml",
  ...
  dispatchEvent(event) {
    if (event.type === "niratan-reader-content-changed") {
...
const window = {
  ...
  scrollX: 0,
  scrollY: 0,
  location: { host: "niratan-novel-book.local" },
  visualViewport: null,
  chrome: {
    webview: {
...
const first = window.niratanSelection.getNormalizedOffset(target, targetLocalOffset);
...
const second = window.niratanSelection.getNormalizedOffset(target, targetLocalOffset);
...
document.dispatchEvent(new FakeEvent("niratan-reader-content-changed"));
...
const afterChange = window.niratanSelection.getNormalizedOffset(target, targetLocalOffset);
...
window.niratanSelection.getNormalizedOffset(target, targetLocalOffset);
...
async function verifySasayakiFallbackInvalidation() {
  ...
  assert.equal(typeof window.niratanReader, "undefined");
  assert.equal(typeof window.niratanSasayaki, "undefined");
  ...
  assert.equal(window.niratanSelection.getNormalizedOffset(secondCueNode, "丙丁".length), initialCueOffset);
  assert.equal(
    counters.treeWalkers,
  ...
  const wrappers = collectElements(
    paragraph,
    element => element.className === "niratan-sasayaki-highlight",
  );
  ...
  assert.equal(
    window.niratanSelection.getNormalizedOffset(wrappedTarget, wrappedTarget.textContent.length),
    wrappedOffset,
  ...
  assert.equal(
    window.niratanSelection.getNormalizedOffset(normalizedTarget, markerEnd),
    normalizedExpected,
  ...
  assert.equal(window.niratanSelection.getNormalizedOffset(normalizedTarget, markerEnd), normalizedExpected);
