...
public class AnkiAudioDownloaderTests
{
    ...
    public async Task DownloadAsync_WhenSourceReturnsAudioSourceList_DownloadsResolvedAudioBytes()
    {
        ...
        result.Filename.Should().Be($"hoshi_audio_{Sha256Hex(mp3Bytes)}.mp3");
    }
    ...
    public async Task DownloadAsync_UsesStableContentHashFilenameAndInfersExtensionFromContentType()
    {
        ...
        first!.Filename.Should().Be($"hoshi_audio_{Sha256Hex(audioBytes)}.ogg");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), "hoshi-anki-audio-tests", Guid.NewGuid().ToString("N"));
        try
        {
            ...
            result.Filename.Should().Be($"hoshi_audio_{Sha256Hex(audioBytes)}.mp3");
